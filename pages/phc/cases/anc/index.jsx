...
export default function ANCList() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await axios.get("/api/phc/cases", { params: { category: "ANC", ...filters }});
      setList(res.data || []);
    } catch (err) {
      console.error(err);
      setList([]);
        ...
        <main className="p-6 mt-16">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <Link href="/phc/cases/anc/new" className="px-3 py-2 bg-blue-600 text-white rounded">+ New Case</Link>
                ...
                <div className="space-y-3">
                  {list.map(r => (
                    <div key={r.id} className="bg-white p-4 rounded shadow flex justify-between items-center">
                      <div>
                        <div className="font-medium">{r.patient_name} <span className="text-sm text-gray-500">({r.age})</span></div>
                        <div className="text-sm text-gray-500">Risk: {r.risk_level} · Status: {r.status}</div>
                      </div>
                      <div className="flex gap-2">
                        <Link href={`/phc/cases/anc/${r.id}`} className="text-blue-600">View</Link>
                      </div>
                    </div>
                  ))}
                </div>
